...
bg = pygame.image.load('pictures/arkanoid.png')
wall_hit_sound = mixer.Sound('music/arcanoid_wall.wav')
destroy_sound = mixer.Sound('music/arcanoid_brick.wav')
lose_life_sound = mixer.Sound('music/lose.wav')
font = pygame.font.Font('20326.otf', 36)
...
def main(lvl):
    ...
    while not exit_program:
        screen.blit(bg, (0, 0))
        displaytext('Press "Q" to exit', 140, 580, pygame.Color('yellow'), False)
        displaytext(f'lifes - {life}', 730, 50, pygame.Color('red'), False)
        displaytext(f'scores: {str(scores)}', 100, 50, pygame.Color('white'), False)
        displaytext(f'Press "ESC" to pause', 630, 580, pygame.Color('yellow'), False)
